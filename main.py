...
async def lifespan(app: FastAPI):
    ...
    if _flaresolverr_env and not get_config("proxy.flaresolverr_url"):
        await config.update({
            "proxy": {
                ...
                "refresh_interval": _env_int("CF_REFRESH_INTERVAL", 600),
                "timeout": _env_int("CF_TIMEOUT", 60),
            }
        })
    ...
    statsig_frontend = import_module("app.services.reverse.utils.statsig_frontend")
    statsig_frontend.start()
