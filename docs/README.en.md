...
| `video_config` | object | **Video model only** | Supported: `grok-imagine-1.0-video` / `grok-imagine-1.5` |
...
- `grok-imagine-1.0-video` / `grok-imagine-1.5` support text-to-video and multi-image reference video: pass up to `7` `image_url` blocks and use placeholders like `@图1`, `@图2` in the prompt; the server will replace them with the corresponding `assetId` values.
- `grok-imagine-1.5` requires a Super Grok token and uses the `ssoSuper` token pool; aspect ratio, duration, resolution, and style preset remain customizable.
