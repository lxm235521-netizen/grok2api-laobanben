...
class TokenManager:
    ...
    def get_token_for_video(
        ...
        respect_pool_candidates_order: bool = False,
    ) -> Optional["TokenInfo"]:
        """
        根据视频需求智能选择 Token 池
        ...
        - 否则优先使用 "ssoBasic" 池
        - 当提供 pool_candidates 时，默认仍结合视频规格确定首选池
        - respect_pool_candidates_order=True 时，严格按候选池顺序回退

        Args:
            resolution: 视频分辨率 ("480p" 或 "720p")
            video_length: 视频时长(秒)
            pool_candidates: 候选 Token 池（按优先级）
            respect_pool_candidates_order: 是否严格尊重候选池顺序
        ...
        if pool_candidates and respect_pool_candidates_order:
