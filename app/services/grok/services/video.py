...
class VideoService:
    ...
    async def generate_from_images(
        ...
        for image_url in image_urls:
            image_references.append(await self.create_image_post(token, image_url))
    ...
    async def completions(
        ...
        for attempt in range(max_token_retries):
            ...
            if image_attachments:
                pool_candidates = [
                    pool
                    for pool in (SUPER_POOL_NAME, BASIC_POOL_NAME)
                    if pool in pool_candidates
                ] or pool_candidates
            token_info = token_mgr.get_token_for_video(
                resolution=resolution,
                video_length=video_length,
                pool_candidates=pool_candidates,
                respect_pool_candidates_order=bool(image_attachments),
            )
                ...
                if image_attachments:
                    ...
                    try:
                        for attach_data in image_attachments:
                            ...
                            image_references.append(f"https://assets.grok.com/{file_uri}")
                        prompt_text = VideoService._replace_reference_placeholders(
                            prompt_text, asset_ids
                        )
                        logger.info(
                            f"Images prepared for video: count={len(image_references)}"
                        )
                    finally:
