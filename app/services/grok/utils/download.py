...
class DownloadService:
    ...
    async def render_video(
        ...
        try:
            ...
            logger.warning(f"Video local cache failed, fallback to upstream URL: {e}")
        ...
        if thumbnail_url:
            try:
                ...
                logger.warning(f"Video thumbnail cache failed, continue without poster: {e}")
    ...
    async def download_file(self, file_path: str, token: str, media_type: str = "image") -> Tuple[Optional[Path], str]:
        ...
        async with _get_download_semaphore():
            ...
            async with _file_lock(lock_name, timeout=lock_timeout):
                ...
                max_attempts = max(
                    1, int(get_config("asset.download_stream_retries", 2) or 2)
                )
                ...
                for attempt in range(1, max_attempts + 1):
                    try:
                        ...
                        logger.warning(
                            f"Download stream failed for {file_path}, "
                            f"retry {attempt}/{max_attempts}: {e}"
                        )
