...
DEFAULT_VIDEO_MODEL_ID = "grok-imagine-1.0-video"
...
def _normalize_video_model(value: Optional[str]) -> str:
    model_id = (value or DEFAULT_VIDEO_MODEL_ID).strip()
    ...
    if not model_info or not model_info.is_video:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                f"model must be a supported video model. "
...
def _verify_sse_function_key(request: Request) -> None:
    ...
    key = (request.query_params.get("function_key") or "").strip()
    if key.startswith("Bearer "):
        key = key[7:].strip()
    if not key or not hmac.compare_digest(key, function_key.strip()):
...
async def function_video_start(data: VideoStartRequest):
    ...
    model_id = _normalize_video_model(data.model_id or data.model)
    ...
    return {"task_id": task_id, "model": model_id, "aspect_ratio": aspect_ratio}


@router.get("/video/sse")
async def function_video_sse(request: Request, task_id: str = Query("")):
    _verify_sse_function_key(request)
    ...
    async def event_stream():
        try:
            model_id = str(session.get("model_id") or DEFAULT_VIDEO_MODEL_ID)
            model_info = ModelService.get(model_id)
            if not model_info or not model_info.is_video:
                payload = {
                    "error": f"Video model `{model_id}` is not available.",
            ...
            logger.exception(f"Function video SSE error: {e}")
