...
| `video_config` | object | **视频模型专用配置对象** | 支持：`grok-imagine-1.0-video` / `grok-imagine-1.5` |
...
- `grok-imagine-1.0-video` / `grok-imagine-1.5` 支持文生视频与多图参考视频：可通过多个 `image_url` 传最多 `7` 张参考图，并在文本中使用 `@图1`、`@图2` 这类占位符；服务端会自动替换为对应 `assetId`。
- `grok-imagine-1.5` 需要 Super Grok Token，会使用 `ssoSuper` Token 池；比例、时长、分辨率、风格参数保持可自定义。
